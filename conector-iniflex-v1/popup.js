...
async function refresh(){const d=await send({action:'v1-status'});if(!d.state){s.textContent='Supervisor pronto. Nenhuma execução ativa.';return}
const x=d.state;s.textContent=(x.status==='done'?'✅ ':'⏳ ')+(x.report_id||'concluído')+' · '+x.report_index+'/14\nFase: '+x.phase+'\nConcluídos: '+((x.completed||[]).join(', ')||'nenhum')+(x.last_error?'\nErro: '+x.last_error:'')}
